class CommunityOfflineClient : MissionGameplay
{
	...
	{
		#ifdef EXPANSION_LOOT_SPAWN_POSITION_THINGY
		ExpansionLootPositions_OnKeyPress( key );
		#endif
		...
		if (key == KeyCode.KC_C)
		{
			...
			Print("Camera POS: " + currentCam.GetPosition());
			Print("Camera ORI: " + currentCam.GetOrientation());
			Print("Camera FOV: " + currentCam.GetCurrentFOV());
		}
